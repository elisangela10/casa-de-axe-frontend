...
import { getGirasEndpoint, listGiras, type Gira } from "../services/giraService";
...
const EMPTY = { data: "", titulo: "", guiaCura: "", descricao: "" };
...
export default function Calendario() {
  ...
  const loadGiras = async () => {
    ...
    setError("");
    try {
      const items = await listGiras();
      setGiras(items);
      setUsingLocalData(false);
      writeLocal(STORAGE_KEY, items);
    } catch {
      const localItems = readLocal<Gira[]>(STORAGE_KEY, []);
      setGiras(localItems);
      setUsingLocalData(localItems.length > 0);
      setError(`Não foi possível carregar ${ENDPOINT}. Verifique a API ou tente novamente.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const sorted = useMemo(() => giras
    .filter((gira) => `${gira.titulo} ${gira.guiaCura} ${gira.descricao || ""}`.toLowerCase().includes(query.toLowerCase()))
    .sort((a, b) => new Date(a.data).getTime() - new Date(b.data).getTime()), [giras, query]);
  ...
  const openEdit = (gira: Gira) => { setEditId(gira.id); setFormData({ data: gira.data.slice(0, 16), titulo: gira.titulo, guiaCura: gira.guiaCura, descricao: gira.descricao || "" }); setModalOpen(true); };
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      let saved: Gira;
      if (editId) {
        try { await api.put(`${ENDPOINT}/${editId}`, { id: editId, ...formData }); } catch { /* mantém fallback local */ }
        saved = { id: editId, ...formData };
      } else {
        let responseData: Gira | null = null;
        try { const response = await api.post(ENDPOINT, { id: 0, ...formData }); responseData = response.data; } catch { /* mantém fallback local */ }
        saved = responseData || { id: Date.now(), ...formData };
      }
      const next = editId ? giras.map((item) => item.id === editId ? saved : item) : [saved, ...giras];
      setGiras(next); writeLocal(STORAGE_KEY, next); closeModal();
    } catch { setError("Não foi possível salvar a gira."); } finally { setSaving(false); }
  };

  const remove = async (id: number) => {
    if (!window.confirm("Tem certeza que deseja excluir esta gira?")) return;
    try { await api.delete(`${ENDPOINT}/${id}`); } catch { /* mantém fallback local */ }
    const next = giras.filter((item) => item.id !== id); setGiras(next); writeLocal(STORAGE_KEY, next);
  };
